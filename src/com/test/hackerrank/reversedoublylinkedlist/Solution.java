package com.test.hackerrank.reversedoublylinkedlist;
...
class Result {
    ...
    public static DoublyLinkedListNode reverse(DoublyLinkedListNode node) {
        ...
        values.forEach(doublyLinkedList::insertNode);
        return doublyLinkedList.head;
    }

    private static void collect(ArrayList<Integer> values, DoublyLinkedListNode node) {
        if (node != null) {
            values.add(node.data);
            collect(values, node.next);
        }
    }
