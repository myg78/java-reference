 ...
 */
public class Test3WithSolution {
    ...
    public static void main(String[] args) {
        ...
        // Problem # 3
        assertEquals(Boolean.TRUE, band.hasMembers());

        // Problem # 3 - Empty band
        RockBand newBand = new RockBand("Python Rocks", "");
        assertEquals(Boolean.FALSE, newBand.hasMembers());

        // Problem # 4
        assertEquals(Boolean.TRUE, band.canPerform());

        // Problem # 4 - Empty band
        assertEquals(Boolean.FALSE, newBand.canPerform());
    }
    ...
    private static class RockBand {
        ...
        public List<String> getFirstNamesOfVocalists() {
            return members.stream()
                    .filter(m -> "vocalist".equals(m.getRole()))
                    .map(BandMember::getFirstName)
        ...
        /**
         * TODO Implement this method to return if band has members. Java 8 Optional preferred.
         */
        public Boolean hasMembers() {
            return Optional.ofNullable(members)
                    .map(bandMembers -> !bandMembers.isEmpty())
                    .orElse(false);
        }

        /**
         * TODO Implement this method to return if band has at least a vocalist, rhythm guitar, drums. Java 8 Optional/Streams preferred.
         */
        public Boolean canPerform() {
            List<String> roles = List.of("vocalist", "rhythm guitar", "drums");
            List<String> bandRoles = Optional.ofNullable(members)
                    .stream()
                    .flatMap(bandMembers -> bandMembers.stream())
                    .map(BandMember::getRole)
                    .collect(Collectors.toList());
            return bandRoles.containsAll(roles);
        }

        // alternate solution
        public Boolean canPerform1() {
            List<String> roles = List.of("vocalist", "rhythm guitar", "drums");
            if (Optional.ofNullable(members).isPresent()) {
                List<String> bandRoles = members.stream()
                        .map(BandMember::getRole)
                        .collect(Collectors.toList());
                return bandRoles.containsAll(roles);
            }
